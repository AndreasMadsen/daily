...
DailyClient.prototype.log = function (level, subject, when, callback) {
  if (subject === undefined) {
    throw new Error('subject can not be undefined');
  }
  ...
  (function reconnect(previuseTrys) {
    ...
    self._attemtConnect(function (connected, socket) {
      ...
      if (connected) {
        ...
        self.emit(emitConnect ? 'connect' : 'reconnect');
      }
      // close for real
...
DailyClient.prototype.close = function (callback) {
  ...
  // If the socket is connecting simply wait so we can get a clean close
  if (this._connecting) {
    this.once('connect', runClose);
    this.once('reconnect', runClose);
  } else {
    runClose();
  }

  function runClose() {
    self.removeListener('connect', runClose);
    self.removeListener('reconnect', runClose);
