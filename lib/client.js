...
DailyClient.prototype.log = function (level, subject, when, callback) {
  ...
  var details = {
    'level': level,
    'subject': new Buffer(JSON.stringify(subject)),
...
function endReader(socket, done) {
...
DailyClient.prototype.close = function (callback) {
  ...
  async.parallel([
    function (done) {
      self._writer.close(done);
    },
    function (done) {
      async.forEach(self._readers, endReader, done);
    }
  ], function () {
    self.emit('close');
    callback();
